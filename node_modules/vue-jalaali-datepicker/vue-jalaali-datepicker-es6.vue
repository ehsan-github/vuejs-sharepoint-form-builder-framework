...
<template>
  <div class="cov-vue-date" :class="option.wrapperClass ? option.wrapperClass : {}">
        <i class="el-icon-close" @click="clearValue" style="display: inline-block;position: absolute;margin: 12px -8px;color: darkgray;font-size: 9px;"></i>
...
export default {
  ...
  data () {
    ...
    updated(){
        if (this.value) {
            if (this.value._i) {
                this.date.time = moment(this.value._i, this.value._f).format('jYYYY-jMM-jDD')
            } else {
                this.date.time = this.value.time
            }
        }
    },
  mounted () {
